...
import PageAttendanceHR  from "./PageAttendanceHR";
...
export default function HRDashboard() {
  ...
  return (
    <div className="hr-shell">
        ...
        <nav className="hr-nav">
          {NAV.map(n => (
            ...
            >
              ...
              {n.id === "leaves" && pending > 0 && (
                <span className="hr-nav-badge">{pending}</span>
              )}
              {n.id === "notifications" && unread > 0 && (
                <span className="hr-nav-badge">{unread}</span>
              )}
            </button>
          ))}
        </nav>
      ...
      {/* Mobile overlay */}
      {sidebarOpen && (
        <div className="hr-overlay" onClick={() => setSidebarOpen(false)} />
      )}
      ...
      {/* ── Floating AI (bottom-right, always visible) ── */}
      <FloatingAI />
