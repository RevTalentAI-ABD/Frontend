...
export async function getDashboardSummary() {
  ...
  const res = await fetch(
    `${BASE_URL}/manager/dashboard-summary`,
    {
      headers: authHeaders()
    }
  );
...
export async function getAttendanceHrSummary(
  from,
  to
) {
...
// ✅ ONLY CHANGE: axiosAuth attaches Bearer token automatically
...
// ─── Employee API ────────────────────────────────────────────────────────────
// ✅ CHANGED: axios → axiosAuth (adds token)
...
export const hrAPI = {
  ...
  // Change department
  changeDepartment: async (data) => {

    return axiosAuth.put(
      `/hr/change-department`,
      data
    );
  }
